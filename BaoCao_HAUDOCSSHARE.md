...
* **Đối tượng nghiên cứu:** Quy trình đăng tải, kiểm duyệt, phân loại, tìm kiếm và chia sẻ các nguồn học liệu (tài liệu, đề thi, video bài giảng) trực tuyến giữa sinh viên Học viện Kỹ thuật Quân sự (MTA).
* **Phạm vi nghiên cứu:** Xây dựng một ứng dụng web hoàn chỉnh có tên gọi HAUDOCSSHARE sử dụng công nghệ ASP.NET Core 10, C# phối hợp với hệ quản trị cơ sở dữ liệu SQLite trong khuôn khổ môn học Công nghệ Web.

### 1.6 Tổng quan
Chương 1 đã làm rõ thực trạng tiếp cận tài nguyên học tập của sinh viên MTA, từ đó xây dựng các yêu cầu chức năng (đăng nhập, tìm kiếm, đăng tải, bình luận, kiểm duyệt) và phi chức năng (bảo mật, hiệu năng, Claymorphism & Glassmorphism UI) thiết yếu cho hệ thống. Đây là những cơ sở lý thuyết và khảo sát thực tế vô cùng quan trọng để nhóm nghiên cứu định hình giải pháp công nghệ ở các chương tiếp theo.
...
### 2.1 Front-end
Giao diện người dùng của hệ thống HAUDOCSSHARE được xây dựng bằng các công nghệ cốt lõi hiện đại nhằm đem đến trải nghiệm tối ưu nhất:
* **HTML5 & CSS3:** Sử dụng để thiết lập cấu trúc trang web chuẩn SEO và định dạng giao diện tùy biến, áp dụng các thuộc tính nâng cao như `backdrop-filter: blur()` để tạo hiệu ứng kính mờ (Glassmorphism) và đổ bóng phức hợp đa lớp để tạo giao diện nổi 3D (Claymorphism).
* **JavaScript & jQuery:** Đóng vai trò cốt lõi trong việc điều khiển tương tác động như chuyển đổi chế độ giao diện sáng/tối (Dark/Light mode) dựa trên `localStorage`, xử lý bất đồng bộ AJAX khi sinh viên đánh giá sao và viết bình luận cho tài liệu mà không cần tải lại toàn bộ trang.
* **Bootstrap 5:** Thư viện CSS hỗ trợ thiết kế đáp ứng (Responsive Design) mạnh mẽ với hệ thống lưới (Grid system) linh hoạt, giúp giao diện hiển thị hoàn hảo trên mọi kích thước màn hình từ điện thoại di động đến máy tính để bàn.
* **Bootstrap Icons:** Cung cấp bộ icon dạng vector chất lượng cao, đồng bộ với phong cách thiết kế hiện đại của hệ thống.

### 2.2 Back-end
Phần máy chủ và xử lý logic nghiệp vụ của hệ thống được vận hành bởi các công nghệ nền tảng mạnh mẽ của Microsoft:
* **ASP.NET Core 10 MVC:** Khung ứng dụng web nguồn mở hiệu năng cao, tổ chức mã nguồn theo mô hình Model-View-Controller giúp tách biệt rõ ràng các nhiệm vụ logic, giao diện và cơ sở dữ liệu.
* **C#:** Ngôn ngữ lập trình hiện đại, hướng đối tượng mạnh mẽ, cung cấp hiệu năng xử lý cực tốt và các cú pháp tối giản an toàn (nullable reference types, pattern matching).
* **Entity Framework Core (EF Core):** Trình ánh xạ đối tượng-quan hệ (ORM) Code-First giúp lập trình viên tương tác với cơ sở dữ liệu hoàn toàn bằng đối tượng C#, tự động khởi tạo và cập nhật cấu trúc cơ sở dữ liệu qua các bản Migration.
* **SQLite Database:** Hệ quản trị cơ sở dữ liệu quan hệ cục bộ gọn nhẹ, không cần thiết lập máy chủ phức tạp, lưu trữ toàn bộ dữ liệu của hệ thống HAUDOCSSHARE trong một tệp tin duy nhất (`documentshare.db`), bảo đảm tốc độ truy xuất nhanh và dễ dàng di động.
* **Bảo mật và Xác thực:** Áp dụng cơ chế lưu trữ Session ở mức máy chủ để theo dõi quyền truy cập của người dùng (Admin/User), bảo đảm mật khẩu được mã hóa an toàn bằng thuật toán SHA256 kết hợp chuỗi muối (salt) ngẫu nhiên trước khi lưu trữ vào cơ sở dữ liệu.

---

## CHƯƠNG 3: PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG

### 3.1 Kiến trúc tổng thể ứng dụng
...
Sơ đồ Use Case tổng quát chỉ ra mối quan hệ tương tác giữa hai nhóm đối tượng chính (Sinh viên/User và Quản trị viên/Admin) đối với các tính năng nghiệp vụ của hệ thống HAUDOCSSHARE:
...
graph LR
    ...
    subgraph Hệ thống HAUDOCSSHARE
        UC1((Đăng ký / Đăng nhập))
        UC2((Tìm kiếm & Lọc tài liệu))
        UC3((Đăng tải tài liệu))
        UC4((Tải xuống tài liệu))
        UC5((Bình luận & Đánh giá))
        UC6((Kiểm duyệt tài liệu))
        UC7((Quản lý thành viên))
        UC8((Quản lý danh mục))
        UC9((Theo dõi thống kê))
    end
...
* **Sinh viên (User):** Thực hiện các hành vi cơ bản như đăng ký, đăng nhập, tìm kiếm tài liệu học tập theo các tiêu chí (ngành, môn học, tác giả, tag), đăng tải tài liệu chia sẻ mới (chờ duyệt), tải xuống tài liệu và viết bình luận, chấm điểm đánh giá chất lượng tài liệu.
* **Quản trị viên (Admin):** Có toàn quyền kiểm soát hoạt động của hệ thống bao gồm duyệt/từ chối phê duyệt tài liệu mới, quản lý thông tin thành viên (kích hoạt/khóa tài khoản), cấu hình danh mục môn học/ngành học và theo dõi biểu đồ thống kê tương tác.

### 3.3 Class diagram
Sơ đồ lớp (Class Diagram) thể hiện cấu trúc dữ liệu tĩnh và mối quan hệ kết hợp giữa các đối tượng thực thể cốt lõi trong hệ thống HAUDOCSSHARE:
...
classDiagram
    ...
    Category "1" --> "*" Document : chứa
    Document "1" --> "*" Comment : nhận
```

Mỗi tài liệu (`Document`) bắt buộc phải thuộc về một danh mục môn học (`Category`) cụ thể và có thể nhận được nhiều bình luận đánh giá (`Comment`) từ phía người học.

### 3.4 Activity diagram
Sơ đồ hoạt động (Activity Diagram) thể hiện quy trình đăng tải và kiểm duyệt tài liệu từ khi sinh viên tải tệp lên cho đến khi tài liệu được xuất bản chính thức hoặc bị từ chối:

```mermaid
flowchart TD
    Start([Bắt đầu]) --> Upload[Sinh viên tải tài liệu / nhập link Drive]
    Upload --> Validate{Thông tin hợp lệ?}
    Validate -- Không --> Error[Hiển thị thông báo lỗi] --> Upload
    Validate -- Có --> Save[Lưu file vào hệ thống / chuyển đổi link Drive]
    Save --> Pending[Đặt trạng thái IsApproved = false]
    Pending --> Queue[Đưa vào hàng đợi chờ duyệt của Admin]
    Queue --> AdminReview{Admin phê duyệt?}
    AdminReview -- Đồng ý --> Approve[Cập nhật IsApproved = true]
    Approve --> Public[Hiển thị tài liệu công khai] --> End([Kết thúc])
    AdminReview -- Từ chối --> Reject[Xóa file vật lý trên máy chủ & xóa bản ghi CSDL]
    Reject --> End
```

### 3.5 Sequence diagram
Sơ đồ tuần tự (Sequence Diagram) mô tả luồng tương tác thời gian thực giữa các thành phần giao diện, bộ điều khiển, cơ sở dữ liệu và hệ thống lưu trữ tệp khi người dùng thực hiện yêu cầu tải xuống một tài liệu:
...
sequenceDiagram
    ...
    Student->>Ctrl: Yêu cầu tải xuống (Document ID)
    Ctrl->>Db: Tìm kiếm tài liệu theo ID
    Db-->>Ctrl: Trả về đối tượng Document (FilePath)
    alt Tài liệu không tồn tại
        Ctrl-->>Student: Trả về trang lỗi 404
    else Tài liệu tồn tại
        Ctrl->>Disk: Kiểm tra sự tồn tại của tệp trên máy chủ
        alt Tệp bị lỗi/mất
            Ctrl-->>Student: Báo lỗi tệp không khả dụng
        else Tệp hợp lệ
            Ctrl->>Db: Tăng DownloadCount +1
            Ctrl->>Db: Lưu thay đổi (SaveChangesAsync)
            Db-->>Ctrl: Xác nhận lưu thành công
            Ctrl->>Disk: Đọc luồng dữ liệu tệp (File Stream)
            Disk-->>Ctrl: Trả về stream file
            Ctrl-->>Student: Gửi tệp tải xuống (FileResult)
        end
    end
```

### 3.6 Database (Database Schema)
Hệ thống sử dụng cơ sở dữ liệu SQLite gọn nhẹ nhưng mạnh mẽ. Dưới đây là chi tiết các bảng và mối quan hệ thực thể:

#### 3.6.1 Bảng `Users` (Người dùng)
...
#### 3.6.3 Bảng `Categories` (Danh mục ngành học)
...
#### 3.6.4 Bảng `Comments` (Bình luận và Đánh giá)
...
#### 3.6.5 Sơ đồ mối quan hệ thực thể (ERD)
...
## CHƯƠNG 4: TRIỂN KHAI CHI TIẾT CÁC TÍNH NĂNG

### 4.1 Thiết kế và Xử lý Cơ sở dữ liệu (Models & EF Core)
...
### 4.2 Lập trình các Bộ điều khiển (Controllers Logic)

#### 4.2.1 Xử lý đăng tải và Xem trước video Google Drive
...
#### 4.2.2 Quy trình phê duyệt tài liệu (Admin Controller)
...
#### 4.2.3 Hệ thống Đăng ký và Đăng nhập (Account Security)
...
### 4.3 Thiết kế Giao diện người dùng (UI/UX Design)
...
#### 5.1.2 Về mặt trải nghiệm người dùng
